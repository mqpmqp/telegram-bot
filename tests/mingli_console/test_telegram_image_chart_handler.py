...
class FakeFile:
    ...
    async def download_to_drive(self, custom_path: str) -> None:
        ...
        if self.fail is not None:
            raise self.fail
        Path(custom_path).write_bytes(b"synthetic-image")
...
class TelegramImageChartHandlerTests(unittest.TestCase):
    ...
    @staticmethod
    def update_for(
        *, user_id: str = "42", photos: list[FakeMedia] | None = None, document: FakeMedia | None = None, text: str | None = None
    ):
    ...
    def test_top_level_candidates_take_precedence_over_analysis(self) -> None:
        ...
        candidate = self.console.sessions["42"].data["candidate"]
        self.assertEqual("甲子", candidate["year_pillar"])
        self.assertEqual([], self.runtime.calls)

    def test_wrapped_candidate_full_confirmation_calls_runtime_once(self) -> None:
        ...
        for value in (
            ...
            self._run_text(value)
        self.assertEqual([], self.runtime.calls)

        self._run_text("确认并分析")
