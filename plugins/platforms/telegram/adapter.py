...
_MINGLI_IMAGE_VISION_PROMPT = """Read only the information visibly printed in this BaZi/Four Pillars chart image. Return JSON with success and a candidates object containing year_pillar, month_pillar, day_pillar, hour_pillar, day_master, and gender. Each candidate field must contain value, confidence, source, and warning. Use source=visible only for text plainly visible in the image. Do not infer, derive, complete, normalize, or guess any missing, blurred, or conflicting value. Set a warning for uncertain content and omit fields that are not visible. Do not return names, account identifiers, contact details, image bytes, or any prose outside the JSON object."""


def _mingli_json_object_once(value: object) -> dict[str, object] | None:
    ...
    def decode(value: object) -> dict[str, object] | None:
        if isinstance(value, str):
            ...
            if text.startswith(fence):
                text = re.sub(r"\A" + re.escape(fence) + r"(?:json)?\s*", "", text, count=1, flags=re.IGNORECASE)
                text = re.sub(r"\s*" + re.escape(fence) + r"\Z", "", text, count=1)
...
def check_telegram_requirements() -> bool:
    ...
    global ApplicationHandlerStop, ContextTypes, filters, ParseMode, ChatType, HTTPXRequest
    ...
    try:
        ...
        from telegram.ext import (
            ...
            ContextTypes as _CT, ApplicationHandlerStop as _AHS, filters as _filters,
        )
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from mingli_console import MingLiConsole
            self._mingli_console = MingLiConsole(
                lambda chat_id, text: self.send(chat_id, text)
            )
        except Exception as exc:
            self._mingli_console = None
            logger.error("MingLi console disabled: %s", type(exc).__name__)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Register MingLi console handlers ahead of the normal agent path.
            # ApplicationHandlerStop prevents console messages from becoming
            # ordinary Hermes prompts while preserving all unrelated traffic.
            if self._mingli_console is not None:
                self._app.add_handler(TelegramMessageHandler(
                    filters.COMMAND, self._handle_mingli_command, block=True
                ), group=-1)
                self._app.add_handler(TelegramMessageHandler(
                    filters.TEXT & ~filters.COMMAND, self._handle_mingli_text, block=True
                ), group=-1)
                self._app.add_handler(TelegramMessageHandler(
                    filters.PHOTO | filters.Document.IMAGE, self._handle_mingli_media, block=True
                ), group=-1)
    ...
    async def _handle_mingli_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        command_name = msg.text.split()[0].split("@", 1)[0].lower()
        if command_name == "/whoami":
            await self._mingli_console.whoami(user_id, chat_id, str(getattr(chat, "type", "unknown")))
            raise ApplicationHandlerStop()
        handled = await self._mingli_console.command(user_id, chat_id, msg.text)
        if handled:
            raise ApplicationHandlerStop()

    async def _handle_mingli_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        if self._mingli_console is None:
            return
        msg = self._effective_update_message(update)
        if not msg or not getattr(msg, "text", None):
    ...
    async def _handle_mingli_media(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        if self._mingli_console is None:
            return
        msg = self._effective_update_message(update)
        if not msg:
